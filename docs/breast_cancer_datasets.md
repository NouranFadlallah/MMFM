...
- **Pathologically confirmed** — whether the source materials state that case labels (benign/malignant/diagnosis) were confirmed via a pathological test (biopsy, histopathology, cytology/FNA, IHC). Marked **Yes** only where the entry's own text says so explicitly (e.g. "biopsy-proven," "pathology-confirmed," "histopathological diagnosis"); **Not stated** where the source text reviewed here doesn't say either way (this does not mean the underlying data lacks pathological confirmation — only that this survey didn't find an explicit statement); **N/A** where labels are not diagnostic (phantoms, tissue-class-only, reproducibility data) or the dataset's own modality *is* the pathology exam (Section 5); and a qualified note where confirmation applies to a treatment-response outcome (e.g. pathologic complete response, pCR) rather than the original diagnosis.
...
- Pathologically confirmed: Not stated
...
- Pathologically confirmed: Yes — "pathology-confirmed benign/malignant"
...
- Pathologically confirmed: Yes — "biopsy-confirmed" (also reflected in the paper title, "An Online Mammography Database with Biopsy Confirmed Types")
...
- Pathologically confirmed: Yes — "biopsy-confirmed"
...
- Pathologically confirmed: Yes, for the cancer-positive cases — "149 biopsy-confirmed cancer-positive cases" (the remainder are negative/false-positive screening outcomes, not biopsy-confirmed)
...
- Pathologically confirmed: Not stated in this imaging entry (see the linked clinical-data companion in [Section 6](#6-clinical-genomic--biomarker-data), which does state biopsy/pathology results are linked)
...
- Pathologically confirmed: Yes — "pathology-confirmed outcome"
...
- Pathologically confirmed: Yes — "clinical/biopsy labels"
...
- Pathologically confirmed: Yes — "histopathological diagnosis (33 subtypes)"
...
- Pathologically confirmed: Yes — "biopsy-proven"
...
- Pathologically confirmed: Yes — "all 99 WHO histopathology categories" / "histopathology category" label
...
- Pathologically confirmed: Yes — "pathology-verified diagnosis"
...
- Pathologically confirmed: Not stated — "dual-pathologist annotation" refers to two pathologists reviewing/labeling the ultrasound video, not a separate tissue biopsy/pathology exam, so it's unclear whether it counts as diagnostic pathological confirmation
...
- Pathologically confirmed: Not stated / unknown (dataset itself unverified)
...
- Pathologically confirmed: Qualified — pCR (pathologic complete response) is a pathology-based *treatment-response* outcome measured after neoadjuvant chemotherapy, not a statement that the original diagnosis was pathologically confirmed
...
- Pathologically confirmed: Qualified — pCR labels are a pathology-based treatment-response outcome, not confirmation of the initial diagnosis
...
- Pathologically confirmed: Not stated explicitly (ER/PR/HER2/Oncotype are typically derived from pathology specimens, but this entry doesn't state that the diagnosis itself was pathologically confirmed)
...
- Pathologically confirmed: Qualified — pCR is available for a subset (70.1%) as a treatment-response outcome; original diagnosis confirmation not stated
...
- Pathologically confirmed: Qualified — pCR is a pathology-based treatment-response outcome; original diagnosis confirmation not stated
...
- Pathologically confirmed: Qualified — "pathological response spreadsheet" covers treatment-response assessment after neoadjuvant chemotherapy, not confirmation of the original diagnosis
...
- Pathologically confirmed: Qualified — pCR/non-responder status is a pathology-based treatment-response outcome; original diagnosis confirmation not stated
...
- Pathologically confirmed: N/A — reproducibility/test-retest data, not diagnostic labels
...
- Pathologically confirmed: N/A — "tissue-class labels only (no cancer diagnosis)"
...
**Explicit gap**: No public dataset of real photon-counting dedicated-breast-CT clinical images exists. The only related public resource is the **2022 AAPM Deep-Learning Spectral CT Grand Challenge** — a *simulated* 2D breast phantom dataset (1,000 train/10 val/100 test pairs), not real acquired data (Pathologically confirmed: N/A — simulated phantom data). DOI: [10.1002/mp.16363](https://doi.org/10.1002/mp.16363). [Challenge page](https://www.aapm.org/GrandChallenge/DL-spectral-CT/).
...
- Pathologically confirmed: Qualified — linked treatment-response (pathological response) data, per the same TCIA collection's MRI entry in [Section 3](#3-mri); original diagnosis confirmation not stated
...
- Pathologically confirmed: N/A — "labels are phantom construction parameters (not real-patient pathology)"
...
**Pathologically confirmed, as a section-wide note**: every dataset below is itself built from histopathology, cytology, or IHC specimens — the imaging modality *is* the pathological test, so a benign/malignant or tumor-subtype label here is inherently pathology-derived (unlike an imaging-only mammography/ultrasound/MRI dataset, where "pathologically confirmed" means a separate biopsy backs up the image label). Per-entry notes below flag the few cases where the released labels are non-diagnostic (registration points, IHC scores only) rather than a diagnosis.
...
- Pathologically confirmed: N/A for diagnosis — labels are registration landmarks, not a benign/malignant call
...
- Pathologically confirmed: Yes (inherent — the dataset consists of histopathology/IHC slides themselves)
...
- Pathologically confirmed: Yes (inherent — H&E histopathology classes are the diagnosis itself)
...
- Pathologically confirmed: Yes (inherent — core-needle biopsy WSIs, and grade/subtype/ER/PR/HER2/Ki67 are pathology-derived readouts)
...
- Pathologically confirmed: Yes (inherent — H&E histopathology subtyping is the diagnosis itself)
...
- Pathologically confirmed: Yes (inherent — H&E histopathology, from surgical biopsy specimens, is the diagnosis itself)
...
- Pathologically confirmed: N/A for diagnosis — cellular-structure annotations supporting grading, not a benign/malignant diagnosis label
...
- Pathologically confirmed: Yes (inherent — H&E lymph-node histopathology metastasis detection is a pathology exam)
...
- Pathologically confirmed: Yes (inherent — H&E + IHC lymph-node histopathology, verified against cytokeratin IHC)
...
- Pathologically confirmed: Yes (inherent — WSI histopathology slides), though this entry describes proteogenomic linkage rather than a diagnosis label
...
- Pathologically confirmed: Yes, for HER2 status (ground truth is IHC, a pathology test) — not a benign/malignant diagnosis label
...
- Pathologically confirmed: Yes, for HER2 status (IHC score is itself the pathology test) — not a benign/malignant diagnosis label
...
- Pathologically confirmed: Yes, for HER2 status (IHC score) — not a benign/malignant diagnosis label
...
- Pathologically confirmed: Yes (inherent — H&E histopathology IDC-positive/negative is the diagnosis itself)
...
- Pathologically confirmed: Yes, for HER2/TNBC status (IHC-based); raw WSI not published so this is inferred from patient categorization rather than confirmed against released images
...
- Pathologically confirmed: N/A for diagnosis — mitotic-cell count annotations, not a benign/malignant diagnosis label
...
- Pathologically confirmed: N/A for diagnosis — atypia/mitosis scoring, not a benign/malignant diagnosis label
...
- Pathologically confirmed: Yes (inherent — H&E lymph-node histopathology metastasis detection, derived from CAMELYON16)
...
- Pathologically confirmed: N/A for diagnosis — cellularity/cell-type annotations (post-neoadjuvant-treatment specimens), not a benign/malignant diagnosis label, though inherently histopathology-derived
...
- Pathologically confirmed: Yes (inherent — H&E axillary lymph-node histopathology metastasis status)
...
- Pathologically confirmed: Yes (inherent — H&E histopathology, IDC/ILC subtype is the diagnosis itself)
...
- Pathologically confirmed: N/A for diagnosis — TIL scoring/tissue segmentation, not a benign/malignant diagnosis label, though inherently histopathology-derived
...
- Pathologically confirmed: N/A for diagnosis — proliferation/mitotic scoring, not a benign/malignant diagnosis label
...
- Pathologically confirmed: Yes (inherent — IHC + H&E histopathology slides)
...
- Pathologically confirmed: Yes (inherent — core-needle biopsy H&E histopathology; grade/ER/PR/HER2/pCR/RCB are pathology-derived readouts)
...
- Pathologically confirmed: Yes (inherent — FNA cytology is itself a pathological diagnostic test)
...
- Pathologically confirmed: Not stated as a whole — "prior biopsy" and "diagnostic outcome" are recorded fields, but the entry doesn't say every case's outcome label is pathology-confirmed
...
- Pathologically confirmed: Not stated explicitly in this entry (see the histology-slide companion in [Section 5](#5-pathology--histopathology), which is inherently pathology-derived)
...
- Pathologically confirmed: Not stated explicitly, though ER/PR/HER2/Ki67 and Nottingham grade are typically pathology-derived readouts
...
- Pathologically confirmed: Not stated explicitly, though ER/PR/HER2 and stage/grade are typically pathology-derived readouts
...
- Pathologically confirmed: Yes — "biopsy/pathology results, cancer diagnosis" explicitly linked
...
- Pathologically confirmed: Yes — "biopsy, histology" are explicitly listed as key variables
...
- Pathologically confirmed: Not stated explicitly in this entry — "tumor site/morphology" reflects cancer-registry coding conventions that generally require pathologic confirmation for case inclusion, but this survey found no explicit statement to that effect
...
- Pathologically confirmed: Not stated explicitly in this clinical/genomic entry, though "pathologic stage" and "histologic type" are pathology-derived fields (see the WSI-based pathology entry above in [Section 5](#5-pathology--histopathology))
...
- Pathologically confirmed: Yes — features are computed from digitized fine-needle-aspirate (FNA) cytology images, and the "diagnosis" label is the cytopathology reading of that specimen
